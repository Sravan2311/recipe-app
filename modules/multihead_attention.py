...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask_future_timesteps=False,
        ...
        the key by passing a **boolean** mask (`key_padding_mask`) with shape:
        batch x src_len, where padding elements are indicated by True.
        """
        ...
        if key_padding_mask is not None:
            # Defensive: accept legacy uint8/byte and cast to bool
        ...
        if key_padding_mask is not None:
            ...
            attn_weights = attn_weights.float().masked_fill(
                key_padding_mask.unsqueeze(1).unsqueeze(2),  # already bool
                float('-inf'),
